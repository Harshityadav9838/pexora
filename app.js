...
const TOOLS = [
  ...
  { id:'pdf-to-word', icon:'W', name:'PDF to Word',        desc:'Requires a server-side conversion engine.', backend:true },
  { id:'word-to-pdf', icon:'W', name:'Word to PDF',        desc:'Requires a server-side conversion engine.', backend:true },
  { id:'html-to-pdf', icon:'</>', name:'HTML to PDF',      desc:'Requires a sandboxed server-side renderer.', backend:true },
...
function renderBackendNotice(shell, body, tool) {
  body.appendChild(el('div', { class:'security-notice' }, [
    el('span', { text:'⚠' }),
    el('div', {}, [
      el('div', { text: `${tool.name} is disabled in this build.` , style:'color:var(--text-hi); font-weight:600; margin-bottom:4px;'}),
      el('div', { text: 'This conversion needs a real document-rendering engine that only exists server-side. Rather than fake a result, this tool is turned off here. A reviewed, isolated-worker architecture for it is included in backend-scaffold/ — deploy that, then this button lights up.' }),
    ])
  ]));
}

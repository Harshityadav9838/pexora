...
## Backend-enabled tools
The optional `backend-scaffold/` service now contains runnable API routes for PDF↔Word, HTML→PDF and PDF password protection/unlocking. These are intentionally separate from browser-only tools so files are uploaded only when a server-backed tool is explicitly run.
